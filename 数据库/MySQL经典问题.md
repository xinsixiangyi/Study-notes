...
而要确定一个查询是否是覆盖索引，我们只需要explain sql语句看==Extra==的结果是否是“Using index”即可。
...
用户本来应该读取到id=1的用户age应该是10，结果读取到了其他事务还没有提交的事务，结果读取结果age=20，这就是==脏读==。
...
默认隔离级别都是第二级别的，要想彻底防止幻读需要提升手动设置隔离级别到最高级别。
...
我们每行数实际上隐藏了两列，一个是行的创建版本，一个是行的删除（过期）版本。具体的版本号（trx_id）在 information_schema.INNODB_TRX 表中。版本号（trx_id）随着每次事务的开启自增。
...
slave记录自己的binglog
![img](https://img-blog.csdnimg.cn/20200913223011592.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L20wXzQ1MjcwNjY3,size_16,color_FFFFFF,t_70#pic_center)
